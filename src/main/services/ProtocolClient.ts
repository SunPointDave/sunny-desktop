...
export const SUNNY_PROTOCOL = 'sunny'

export function registerProtocolClient(app: Electron.App) {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      app.setAsDefaultProtocolClient(SUNNY_PROTOCOL, process.execPath, [process.argv[1]])
    }
  }

  app.setAsDefaultProtocolClient(SUNNY_PROTOCOL)
}

export function handleProtocolUrl(url: string) {
  if (!url) return
  // Process the URL that was used to open the app
  // The url will be in the format: sunny://data?param1=value1&param2=value2
...
const DESKTOP_FILE_NAME = 'sunny-url-handler.desktop'

/**
 * Sets up deep linking for the AppImage build on Linux by creating a .desktop file.
 * This allows the OS to open sunny:// URLs with this App.
 */
...
Terminal=false
Type=Application
MimeType=x-scheme-handler/${SUNNY_PROTOCOL};
NoDisplay=true
`
