...
class BackupManager {
  private tempDir = path.join(app.getPath('temp'), 'sunny', 'backup', 'temp')
  private backupDir = path.join(app.getPath('temp'), 'sunny', 'backup')
  ...
  async backupToWebdav(_: Electron.IpcMainInvokeEvent, webdavConfig: WebDavConfig) {
    const filename = webdavConfig.fileName || 'sunny.backup.zip'
  ...
  async backupToS3(_: Electron.IpcMainInvokeEvent, s3Config: S3Config) {
    ...
    const timestamp = new Date()
      ...
      .replace(/[-:T.Z]/g, '')
      .slice(0, 14)
    const filename = s3Config.fileName || `sunny.backup.${deviceName}.${timestamp}.zip`
  ...
  private async restoreDirect(): Promise<void> {
    ...
    try {
      ...
      // Validate appName to ensure backup is from Sunny
      if (metadata.appName !== 'Sunny') {
        throw new Error('This backup file is not from Sunny and cannot be restored')
      }
  ...
  async restoreFromS3(_: Electron.IpcMainInvokeEvent, s3Config: S3Config) {
    const filename = s3Config.fileName || 'sunny.backup.zip'
  ...
  async createLanTransferBackup(
    ...
    const fileName = `sunny.${timestamp}.zip`
    const tempPath = path.join(app.getPath('temp'), 'sunny', 'lan-transfer')
  ...
  async deleteLanTransferBackup(_: Electron.IpcMainInvokeEvent, filePath: string): Promise<boolean> {
    try {
      ...
      const tempBase = path.normalize(path.join(app.getPath('temp'), 'sunny', 'lan-transfer'))
